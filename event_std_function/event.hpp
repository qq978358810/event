...
class sigh<Ret(Args...), Allocator> {
    ...
    using alloc_traits = std::allocator_traits<Allocator>;
    using delegate_type = std::function<Ret(Args...)>;
...
class connection {
    ...
    connection(std::function<void(void *)> fn, void *ref)
...
class sink<sigh<Ret(Args...), Allocator>> {
    ...
    template <typename C>
    static void release(C& c,void *signal) {
        sink{*static_cast<signal_type *>(signal)}.disconnect(c);
    }
...
public:
     ...
     */
    template <typename C>
        requires std::convertible_to<C, std::function<Ret(Args...)>>
    connection connect(C&& c) {
        disconnect(std::forward<C>(c));
        signal_or_assert().calls.emplace_back(std::forward<C>(c));
        std::function<void(void*)> conn = [c = std::forward<C>(c)](void* sig) {
            release(c, sig);
        };
        return {conn, signal};
    }
    ...
    template <typename C>
        requires std::convertible_to<C, std::function<Ret(Args...)>>
    void disconnect(C&& c) {
        std::function<Ret(Args...)> func{std::forward<C>(c)};
        disconnect_if([&func](const auto &elem) { return elem.target_type() == func.target_type(); });
    }
